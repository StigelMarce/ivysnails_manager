    ...
    <div class="mx-auto max-w-3xl px-5 py-4 md:p-6">
        <div class="mb-6">
            <div class="flex items-center gap-3 pb-6">
                ...
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white/90">Editar Cliente</h2>
            </div>
        </div>

        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-6 py-5">
                <form action="{{ route('admin.clientes.update', $cliente) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        @if ($cliente->foto)
                            <div>
                                ...
                                <img src="{{ asset('storage/' . $cliente->foto) }}"
                                    class="h-24 w-24 rounded-lg object-cover" alt="{{ $cliente->nombre }}">
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            <select name="sexo" id="sexo"
                                ...
                                <option value="" {{ old('sexo', $cliente->sexo) == '' ? 'selected' : '' }}>Seleccione</option>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">JPG, PNG o GIF (Max. 2MB). Dejar vacío para mantener la foto actual.</p>
                            @error('foto')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
